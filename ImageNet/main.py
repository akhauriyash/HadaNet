...
parser.add_argument(
    "--momentum", default=0.90, type=float, metavar="M", help="momentum"
)
parser.add_argument(
    "--weight-decay",
    "--wd",
...
def main():
    ...
    optimizer = torch.optim.Adam(
        model.parameters(), 0.1, weight_decay=args.weight_decay
    )
    ...
    val_loader = torch.utils.data.DataLoader(
        datasets.ImageFolder(
            args.data,
            transforms.Compose(
                [transforms.ToTensor(), normalize, transforms.CenterCrop(input_size),]
            ),
            Train=False,
        ),
        batch_size=16,
