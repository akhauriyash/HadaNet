...
class hbPass(nn.Module):
    def __init__(self, input_channels, output_channels, previous_conv = False, size=0,\
            kernel_size=-1, stride=-1, padding=0, dropout=0, groups=1, Linear=False, bias=True):
...
def conv3x3(in_planes, out_planes, stride=1):
    return hbPass(in_planes, out_planes, kernel_size=3, stride=stride, padding=1, bias=False)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.bn2 = nn.BatchNorm2d(planes)
        self.conv2 = hbPass(planes, planes, kernel_size=3, stride=stride,
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                ...
                hbPass(self.inplanes, planes * block.expansion,
