...
class BinOp:
    def __init__(self, model):
        ...
        self.bin_range = (
            numpy.linspace(start_range, end_range, end_range - start_range + 1)
            .astype("int")
            .tolist()
        )
    ...
    def updateBinaryGradWeight(self):
        for index in range(self.num_of_params):
            ...
            self.target_modules[index].grad.data = (
                m.add(m_add).mul(1.0 - 1.0 / s[1]).mul(n)
            )
            self.target_modules[index].grad.data = self.target_modules[
                index
            ].grad.data.mul(1e9)
