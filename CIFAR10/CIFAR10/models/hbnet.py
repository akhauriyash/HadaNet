...
def binFunc(input, abs_bin=False, signed_bin=False, binmat_mul=False, num=-1):
    binAgg = num
    shape   = input.size()
    # restore = 0
    if(len(shape)==4):
...
class BinActive(Function):
    @staticmethod
    def forward(ctx, input, num):
        ctx.num = num
        output  = binFunc(input, abs_bin=True, num=num)
    ...
    def backward(ctx, grad_output):
        ...
        m_add = binFunc(m_add, signed_bin=True, num=ctx.num).mul(input.sign())
#        if len(s) == 4:
...
class hbPass(nn.Module):
    def __init__(self, input_channels, output_channels, previous_conv = False, size=0,\
            kernel_size=-1, stride=-1, padding=-1, dropout=0, groups=1, Linear=False, num=2):
...
class HbNet(nn.Module):
    def __init__(self, num):
        ...
        self.xnor = nn.Sequential(
                ...
                nn.BatchNorm2d(192, eps=1e-4, momentum=0.1, affine=False),
                hbPass(192, 160, kernel_size=1, stride=1, padding=0, num=num),
                nn.PReLU(),

                nn.BatchNorm2d(160, eps=1e-4, momentum=0.1, affine=False),
                hbPass(160,  96, kernel_size=1, stride=1, padding=0, num=num),
                nn.PReLU(),

                nn.MaxPool2d(kernel_size=3, stride=2, padding=1),

                nn.BatchNorm2d(96, eps=1e-4, momentum=0.1, affine=False),
                hbPass( 96, 192, kernel_size=5, stride=1, padding=2, dropout=0.5, num=num),
                nn.PReLU(),

                nn.BatchNorm2d(192, eps=1e-4, momentum=0.1, affine=False),
                hbPass(192, 192, kernel_size=1, stride=1, padding=0, num=num),
                nn.PReLU(),
                ...
                nn.BatchNorm2d(192, eps=1e-4, momentum=0.1, affine=False),
                hbPass(192, 192, kernel_size=3, stride=1, padding=1, dropout=0.5, num=num),
                nn.PReLU(),
...
#         self.conv1 = hbPass(192, 160, kernel_size=1, stride=1, padding=0, num),
#         self.rl1 = nn.PReLU(),

#         self.bn2 = nn.BatchNorm2d(160, eps=1e-4, momentum=0.1, affine=False),
#         self.conv2 = hbPass(160,  96, kernel_size=1, stride=1, padding=0, num),
#         self.rl2 = nn.PReLU(),
...
#         self.conv4 = hbPass(192, 192, kernel_size=1, stride=1, padding=0, num),
#         self.rl4 = nn.PReLU(),

#         self.bn5 = nn.BatchNorm2d(192, eps=1e-4, momentum=0.1, affine=False),
#         self.conv5 = hbPass(192, 192, kernel_size=1, stride=1, padding=0, num),
#         self.rl5 = nn.PReLU(),
...
#         self.conv7 = hbPass(192, 192, kernel_size=1, stride=1, padding=0, num),
#         self.rl7 = nn.PReLU(),

#         self.bn8 = nn.BatchNorm2d(192, eps=1e-4, momentum=0.1, affine=False),
#         self.conv8 = nn.Conv2d(192,  10, kernel_size=1, stride=1, padding=0, num),
#         self.rl8 = nn.PReLU(),
#         self.mp2 = nn.AvgPool2d(kernel_size=8, stride=1, padding=0, num),
...
#         # self.bin_conv2 = hbPass(20, 50, kernel_size=5, stride=1, padding=0, num)
...
#         self.conv1 = nn.Conv2d(3, 6, kernel_size=5, stride=1, padding=0, num)
#         self.relu  = nn.PReLU()
#         self.pool1 = nn.MaxPool2d(kernel_size=2, stride=2) 
#         self.bn0   = nn.BatchNorm2d(6, eps=1e-4, momentum=0.1, affine=True)
#         self.conv2 = hbPass(6, 16, kernel_size=5, stride=1, padding=0, num)
#         self.pool2 = nn.MaxPool2d(kernel_size=2, stride=2)
