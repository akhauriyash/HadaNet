...
class BinOp:
    ...
    def binarization(self):
        ...
        self.save_params()
        self.binarizeConvParams(self.num)
    ...
    def clampConvParams(self):
        for index in range(self.num_of_params):
            self.target_modules[index].data = self.target_modules[index].data.clamp(
                -1.0, 1.0
            )
    ...
    def binarizeConvParams(self, num):
        for index in range(self.num_of_params):
            n = self.target_modules[index].data[0].nelement()
            s = self.target_modules[index].data.size()
            m = binFunc(self.target_modules[index].data, binmat_mul=True, num=num)
    ...
    def updateBinaryGradWeight(self):
        for index in range(self.num_of_params):
            ...
            n = weight[0].nelement()
            s = weight.size()
            m = binFunc(weight, abs_bin=True, num=self.num)
            m[weight.lt(-1.0)] = 0
            m[weight.gt(1.0)] = 0
            m = m.mul(self.target_modules[index].grad.data)
            m_add = weight.sign().mul(self.target_modules[index].grad.data)
            m_add = binFunc(m_add, signed_bin=True, num=self.num)
            # if len(s) == 4:
            ...
            self.target_modules[index].grad.data = (
                m.add(m_add).mul(1.0 - 1.0 / s[1]).mul(1e9)
            )
